...
# Profile zfrac
zfrac <- seq(0.1, 0.9, 0.1)

x <- r4ss::profile(
  file.path(ss_home, "m37_flatselsurv_dw_zfracprof"),
...
zfrac_prof <- r4ss::SSgetoutput(
  ...
  dirvec = file.path(ss_home, "m37_flatselsurv_dw_zfracprof")
)
...
g2 <- SS3_prof_like(zfrac_prof, zfrac, xval = "steep") +
  labs(x = "Steepness") +
  coord_cartesian(xlim = c(0.2, 0.6))

g <- ggpubr::ggarrange(g1, g2, common.legend = TRUE)
...
g <- SS3_prof_like(R0_prof, logR0, by_fleet = FALSE, xval = "dep") +
  labs(x = expression(B[2023]/B[0]))
ggsave("figs/ss3/prof/like_R0_dep.png", g, height = 3, width = 5)

g <- SS3_prof_like(R0_prof, logR0, by_fleet = TRUE, component = c("Length_like", "Surv_like")) +
