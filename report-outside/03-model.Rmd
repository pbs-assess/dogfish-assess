...
The base model estimated the unfished recruitment $R_0$, productivity parameter $z_\textrm{frac}$, and selectivity parameters for the various fisheries and surveys (Table \@ref(tab:ss3-par)).
...
The maximum age in the model is 70 years.
...
$1/\phi_0$ is the unfished replacement line and is intrepretable as the survival of pups to the recruit life stage when the stock is at unfished levels.
...
$z_0$ is the instantaneous rate when the stock is at $S_0$.
$z_\textrm{min}$ is the mortality rate as the stock approaches zero, and is parameterized as a $z_\textrm{frac}$ proportion of $z_0$, i.e., $z_\textrm{min} = z_0(1 - z_\textrm{frac})$.
...
As a result, there is an implied upper bound to steepness, the recruitment relative to $R_0$ when the stock is at $0.2 S_0$.
Steepness cannot exceed $1/(5\phi_0)$ [@taylor2013].
...
The estimated parameters for the stock-recruit relationship are $R_0$ and $z_\textrm{frac}$.
A uniform prior was used for $R_0$ and a beta prior was used for $z_\textrm{frac}$.

For fisheries and/or surveys with length composition, either six parameters were estimated if the female selectivity was assumed to be flat-topped or seven parameters if dome-shaped.
...
The fleet structure of the fisheries in the model was broadly organized in terms of fishing gear and retention behavior (landings and discards).

Data
Likelihoods
Reweighting length comps.

The base model was fitted with the following structure:

### Sensitivity analyses

## MCMC

After minimizing the objective function (the negative log posterior), the posterior was sampled using MCMC.

We proceeded with the MCMC for a subset of the sensitivity models.

## Tables

```{r ss3-par, results="asis"}
ss3_par <- readxl::read_excel(here::here("tables/ss3_par_annotate_dogfish.xlsx")) %>%
...
csasdown::csas_table(
  ...
  font_size = 8,
  format = "latex",
  caption = "Parameters reported from SS3 for model A1.",
  align = c("l", "l", "r", "r", "l")
) %>%
...
csasdown::csas_table(
  ss3_like,
  caption = "Summary of SS3 likelihood and prior equations. Variables $a$ indexes age, $y$ indexes year, $f$ indexes fishery, $i$ indexes survey, and $s$ indexes sex. The circumflex symbol denotes an estimate.",
  format = "latex",
  align = c("l", "l")
) %>%
