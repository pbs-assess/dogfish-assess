...
d_4b5abcde_trawl <- d_4b5abcde |>
  filter(year >= 1996 & gear %in% c("Bottom trawl", "Midwater trawl"))

d_4b5abcde_ll <- d_4b5abcde |>
  filter(year >= 2007 & gear %in% c("Hook and line", "Trap", "Unknown/trawl", "Discarded"))

d_catch_modern <- bind_rows(d_4b5abcde_trawl, d_4b5abcde_ll)

# plot_catch(d_4b5abcde_ll)
# plot_catch(d_4b5abcde_trawl)
...
d_1935 <- landings_1935 |>
  transmute(year = Year, `4B` = `4B`, `5ABCDE` = Total)
...
d_1966_ll <- landings_1966_ll |>
  transmute(year = Year, `4B` = `4B`, `5ABCDE` = `Total outside` + `Unknown area`)
...
d_1966_tr <- landings_1966_trawl |>
  transmute(year = Year, `4B` = `4B`, `5ABCDE` = `Total` + `Unknown area`)
