...
test2 <- test |> mutate(matbin = ifelse(sex == 1 & maturity_code <30, "immature", ifelse(sex == 1 & maturity_code >= 30, "mature", ifelse(sex ==2 & maturity_code < 77, "immature", ifelse(sex ==2 & maturity_code >=77, "mature", NA)))))
test2 |> filter(sex %in% c(1,2)) |>
  drop_na(maturity_code) |>
  filter(maturity_code != 0) |>
  ggplot(aes((matbin), length, colour = (matbin))) +
  geom_jitter() +
