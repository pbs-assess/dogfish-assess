...
d %>%
  drop_na(length) %>%
  mutate(sex = ifelse(sex == "F", tr("F", "F"), tr("M", "M"))) %>%
  ggplot() +
  geom_freqpoly(aes(length, group = sex, col = sex)) +
  facet_wrap(vars(year)) +
  labs(x = tr("Length", "Longueur"), y = tr("Frequency", "Fréquence"), col = tr("Sex", "Sexe"))
ggsave(fig_path("biosample-iphc.png"), width = 6, height = 4, dpi = 150)
...
g <- gfplot::plot_lengths(lengths_iphc, show_year = "all", french = FRENCH)
ggsave(fig_path("lengths-iphc.png"), g, width = 4, height = 6)
...
g <- d_coord %>%
  ...
  ggtitle(tr("IPHC - by latitude", "IPHC - par latitude")) +
  labs(x = tr("Length (cm)", "Longueur (cm)"), y = tr("Frequency", "Fréquence"), linetype = tr("Sex", "Sexe")) +
  theme(legend.position = "bottom")
ggsave(fig_path("biosample-iphc-latitude.png"), width = 6, height = 3, dpi = 150)
