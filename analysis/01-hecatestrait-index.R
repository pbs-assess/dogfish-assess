...
if (FRENCH) {
  ...
  dir.create("figs-french/msa", showWarnings = FALSE)
  fig_dir <- "figs-french"
} else {
...
ggplot(grid_hs, aes(UTM.lon, UTM.lat, fill = exp(logbot_depth), colour = exp(logbot_depth))) +
  ...
  labs(colour = tr("Depth (m)", "Profondeur (m)"), fill = tr("Depth (m)", "Profondeur (m)")) +
  coord_fixed()
ggsave(fig_path("msa/depth-grid.png"), width = 4, height = 5)
...
ggplot(dh, aes(expected, observed)) + geom_abline(intercept = 0, slope = 1, colour = "red") + geom_point(size = 2) + xlab(tr("Expected", "Attendu")) + ylab(tr("Observed", "Observé"))
ggsave(fig_path("msa/qq.png"), width = 5, height = 5, dpi = 180)
...
sanity(m)
plot_anisotropy(m)
ggsave(fig_path("msa/aniso.png"), width = 4, height = 4)
...
ind_dl_filtered |>
  ...
  ylab(tr("Relative biomass index", "Indice de biomasse relative")) + xlab(tr("Year", "Année")) +
  ...
  # ggtitle("RW fields")
ggsave(fig_path("msa/index.png"), width = 5, height = 4)
...
ind_iid |>
  ...
  scale_x_continuous(breaks = seq(1984, 2002, 2)) +
  ggtitle("IID")
ggsave(fig_path("msa/index-iid.png"), width = 5, height = 4)
d_sf2 |>
  ggplot() +
...
ggsave(fig_path("msa/cpue-map.png"), width = 7, height = 9)

xx <- group_by(d_sf2, year) |> summarise(mj = mean(julian))
left_join(ind_iid, xx) |> ggplot(aes(mj, est)) + geom_point() + xlab(tr("Julian day", "Jour julien")) +
  ylab(tr("Index value", "Valeur d'indice"))
...
gg <- ggplot(pp, aes(UTM.lon, UTM.lat, fill = plogis(est1) * exp(est2), colour = plogis(est1) * exp(est2))) +
  ...
  theme(panel.spacing = unit(0, "in"),
    legend.position = 'bottom',
    axis.text.x = element_text(angle = 45, vjust = 0.5)) +
  labs(x = "UTM", y = "UTM", colour = tr("Density", "Densité"), fill = tr("Density", "Densité"))
ggsave(fig_path("msa/prediction_grid_density.png"), gg, height = 9, width = 7, dpi = 200)
